package com.github.oscerd.JavaCassandraExample;
...
//		cluster = Cluster.builder().addContactPoint(node).build();
	...
	public void loadData() {
		...
		ResultSet res = session.execute(boundStatement.bind(idAlbum, "DNR", "The gathering", "Testament", tags));
		...
		session.execute(boundStatement.bind(idPlaylist, idAlbum, "DNR", "The gathering", "Testament"));
	}

	public void querySchema() {
		Statement statement = QueryBuilder.select().all().from("simplex", "songs");
		ResultSet results = session.execute(statement);
		System.out.println(String.format("%-50s\t%-30s\t%-20s\t%-20s\n%s", "id", "title", "album", "artist",
				"------------------------------------------------------+-------------------------------+------------------------+-----------"));
		for (Row row : results) {
			System.out.println(String.format("%-50s\t%-30s\t%-20s\t%-20s", row.getUUID("id"), row.getString("title"),
